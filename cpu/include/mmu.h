...
uint32_t consultarTablaSegundoNivel(uint32_t tablaDePaginasPrimerNivel,
		uint32_t pagina);

uint32_t consultarMarco(uint32_t tablaDePaginasSegundoNivel, uint32_t pagina,
		t_op_code codOP);

uint32_t consultarDireccionFisica(uint32_t tablaPaginasPrimerNivelPCB,
		uint32_t direccion_logica, t_op_code codOP);
...
uint32_t obtenerDesplazamiento(uint32_t direccion_logica,
